...
\begin{solbox}
    Vis at en ortogonal liste med vektorer $ \left( u_1,..., u_n \right)  $ er automatisk lineært uavhengig. Vi vet at $ \langle u_j, u_k \rangle_{  } =0   \forall j\neq k $. 

    Anta at $ \left( u_1, ..., u_n \right)  $ er lin. uavh. da er

    \[
        0 = \sum_{ k=1 }^{ n } \alpha_{k}^{} u_k
    \]
    sant hvis og bare hvis $ \alpha_{k}^{} =0 \forall k = 1,..., n $. Multipliser $ u_j $  for en $ j \in \left[ 1, n \right]  $ på begge sider og få


    \begin{align*}
        \langle u_j, \sum_{ k=1 }^{ n } \alpha_{k}^{} u_k \rangle_{} &= u_j * 0 \\
        \sum_{ k=1 }^{ n } \alpha_{k}^{}  \langle u_j, u_k \rangle_{  } &= u_j * 0 \\
    \end{align*}
    
    Og siden $ \left( u_1, ..., u_n \right)  $ er ortogonal, så er dette lik 0 for alle ulike j og k. Det vil si at vi får

    \[
        \alpha_{j}^{} \langle u_j, u_j \rangle_{  } = 0 
    \]

    \[
        \alpha_{j}^{} \norm{u_j}_{}^{2} =0
    \]
    som har eneste løsning at $ \alpha_{j}^{} =0 $ og det er bevist at en ortgonal liste også er lineært uavhengig
